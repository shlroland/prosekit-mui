...
import type { EditorMenuOption } from './types'
...
export type EditorComboboxMenuProps = {
  ...
  onSelect: (option: EditorMenuOption) => void
  onClose?: () => void
  getOptionIcon?: (option: EditorMenuOption, active: boolean) => ReactNode
  getOptionMeta?: (option: EditorMenuOption) => ReactNode
  renderOptionLabel?: (option: EditorMenuOption) => ReactNode
}

function getSelectedOption(options: EditorMenuOption[], value?: string) {
  return options.find((option) => option.id === value) ?? null
}

function itemToStringLabel(option: EditorMenuOption | null) {
  return option?.label ?? ''
}

function itemToStringValue(option: EditorMenuOption | null) {
  return option?.id ?? ''
}

function isItemEqualToValue(itemValue: EditorMenuOption, value: EditorMenuOption) {
  return itemValue.id === value.id
}

function filterOption(option: EditorMenuOption, query: string) {
...
export function EditorComboboxMenu({
  children,
  options,
  value,
  open,
  ...
  function handleValueChange(nextValue: EditorMenuOption | null) {
    if (!nextValue) {
      return
    }
    ...
    >
      ...
      >
        ...
        <ComboboxList className={listClassName}>
          {(option: EditorMenuOption, index: number) => {
            const active = option.id === value || !!option.selected
            const icon = getOptionIcon?.(option, active)
              ?? option.icon
              ?? (active ? <CheckboxCircleLineIcon className="pk:h-4 pk:w-4" /> : null)
            const meta = getOptionMeta?.(option) ?? option.meta ?? option.description ?? option.id
            ...
            return (
              <ComboboxItem
                ...
                className={cn(
                  ...
                  option.destructive && 'pk:text-[var(--editor-danger,#dc2626)]',
                )}
              ...
              >
                ...
                {meta ? (
                  <span className="pk:ml-4 pk:text-[11px] pk:text-[var(--editor-muted-foreground)]">
                    {meta}
                  </span>
                ) : null}
              </ComboboxItem>
