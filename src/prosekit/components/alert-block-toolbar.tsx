...
function AlertToolbarSurface({ children }: { children: ReactNode }) {
  return (
    <div
      className="pk:rounded-lg pk:border pk:border-[var(--editor-border)] pk:bg-[var(--editor-surface)] pk:p-1 pk:shadow-[0_8px_24px_rgba(15,23,42,0.16),0_2px_8px_rgba(15,23,42,0.08)]"
    ...
    >
      ...
      >
        <BlockHandlePopup>
          {alertBlockState ? (
            <AlertToolbarSurface>
              <BlockHandleDraggable editor={editor} className="pk:inline-flex">
                ...
                <Tooltip content={option.label} key={option.value}>
                  <Button
                    ...
                    className={cn(
                      ...
                      option.value === activeVariant && 'pk:bg-[var(--editor-primary-soft)] pk:text-[var(--editor-primary)]',
                    )}
              ...
              {alertTypeOptions.map((option) => (
                <Tooltip content={option.label} key={option.value}>
                  <Button
                    ...
                    className={cn(
                      'pk:h-7 pk:w-7 pk:rounded-md pk:text-[var(--editor-muted-foreground)] pk:hover:bg-[var(--editor-muted)] pk:hover:text-[var(--editor-foreground)]',
                      option.value === activeType && 'pk:bg-[var(--editor-primary-soft)] pk:text-[var(--editor-primary)]',
                    )}
              ...
              <Tooltip content="删除提示块">
                <Button
                  ...
                  onClick={deleteAlert}
                >
                  <DeleteLineIcon className="pk:h-4 pk:w-4" />
                </Button>
              </Tooltip>
            </AlertToolbarSurface>
          ) : null}
